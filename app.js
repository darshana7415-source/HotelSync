...
// clean-shift-page-v243
(function () {
  if (window.cleanShiftPageV243) return;
  window.cleanShiftPageV243 = true;
  ...
  function shiftFor(person, key) {
    var roster = window.dailyRosters || {};
    var entry = roster[key]?.[person.id] || roster[key]?.[person.cloudId] || {};
    var name = entry.shiftName || entry.shift || person.shift || person.defaultShiftType || "10h shift";
    var start = entry.inTime || entry.startTime || entry.in_time || "";
    var end = entry.outTime || entry.endTime || entry.out_time || "";
    var time = start && end ? `${start.slice(0,5)} - ${end.slice(0,5)}` : "Time not set";
    var status = String(entry.status || entry.dayStatus || entry.day_status || "").toLowerCase();
    if (status.includes("leave")) return { name: "Leave", time: entry.dayStatus || "On leave" };
    return { name, time };
  }

  function sortStaff(list) {
    return [...list].sort(function (a, b) {
      var ac = String(a.employeeCode || a.employee_code || "").padStart(6, "0");
      var bc = String(b.employeeCode || b.employee_code || "").padStart(6, "0");
      return ac.localeCompare(bc, undefined, { numeric: true });
    });
  }

  function renderBoard() {
    if (pageNow() !== "shifts") return;
    var host = document.querySelector("#shift-calendar");
    if (!host) return;

    var people = sortStaff(window.staff || []);
    var dates = [dateKey(0), dateKey(1), dateKey(2)];
    ...
    var html = `
      <div class="shift-clean-board">
        <h3>Coming 3 days shifts</h3>
        <p class="form-note">Open a department to see staff shift times.</p>
        ${Object.keys(groups).sort().map(function (dept) {
          return `
            <details class="shift-dept-fold">
              <summary>${dept}</summary>
              <div class="shift-simple-grid">
                <div class="shift-simple-head">Staff</div>
                ${dates.map(d => `<div class="shift-simple-head">${niceDate(d)}</div>`).join("")}
                ${groups[dept].map(function (p) {
                  return `
                    <div class="shift-simple-staff">
                      <strong>${p.employeeCode || p.employee_code || ""} - ${p.name || p.fullName || p.full_name || "Staff"}</strong>
                      <small>${p.role || p.jobTitle || p.job_title || "Staff"}</small>
                    </div>
                    ${dates.map(function (d) {
                      var s = shiftFor(p, d);
                      return `<div class="shift-simple-cell"><strong>${s.name}</strong><span>${s.time}</span></div>`;
                    }).join("")}
                  `;
                }).join("")}
              </div>
  ...
  var oldSync = window.syncCloudDashboard;
  if (typeof oldSync === "function") {
    window.syncCloudDashboard = async function () {
