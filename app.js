...
init();
// ============================================================================
// ENGENIUS LOCATION TRACKING - Add this to the end of app.js
// ============================================================================

// 1. REGISTER PHONE MAC
if (staffDeviceForm) {
  staffDeviceForm.addEventListener("submit", async (e) => {
    e.preventDefault();

    const staffId = deviceStaff?.value;
    const deviceName = document.querySelector("#device-name")?.value;
    const phoneMac = document.querySelector("#device-mac")?.value;

    if (!staffId || !deviceName || !phoneMac) {
    ...
    try {
      ...
      if (!/^([0-9A-F]{2}:){5}([0-9A-F]{2})$/.test(normalizedMac)) {
        showToast("Invalid MAC format. Use: AA:BB:CC:DD:EE:FF", "error");
        return;
      }

      const { error } = await staffSyncDb.updateStaffProfilePhoneMac({
        staffProfileId: staffId,
        phoneMac: normalizedMac
      });
      ...
      showToast(
        `✅ Registered ${deviceName} (${normalizedMac}) to staff`,
        "success"
      );
...
// 2. REFRESH STAFF DEVICE LIST
async function refreshStaffDeviceList() {
  const staffDeviceList = document.querySelector("#staff-device-list");
  if (!staffDeviceList) return;

  try {
    const staffList = await staffSyncDb.getStaffProfiles();

    const devicesHTML = staffList
      .filter(s => s.phone_mac)
      .map(s => `
        <div class="device-item">
          <strong>${s.full_name}</strong>
          <small>${s.employee_code}</small>
          <br/>
          <code>${s.phone_mac}</code>
          <br/>
          <small>${s.job_title || "No title"}</small>
        </div>
      `)
      .join("");

    if (!devicesHTML) {
      staffDeviceList.innerHTML = `
        <p style="color: #666; font-style: italic;">
          No devices registered yet. Use the form above to register staff phone MACs.
        </p>
      `;
      return;
    }

    staffDeviceList.innerHTML = devicesHTML;

  } catch (error) {
    staffDeviceList.innerHTML = `<p style="color: red;">Error loading devices: ${error.message}</p>`;
  }
}

refreshStaffDeviceList();

// 3. AP ZONE MAPPING
if (apMapForm) {
  apMapForm.addEventListener("submit", async (e) => {
    e.preventDefault();

    const apName = document.querySelector("#ap-map-name")?.value;
    const floor = document.querySelector("#ap-map-floor")?.value;
    const zone = document.querySelector("#ap-map-zone")?.value;

    if (!apName || !floor || !zone) {
      showToast("All fields required", "warning");
      return;
    }

    try {
      const { error } = await staffSyncDb.createAPZoneMapping({
      ...
      showToast(`✅ Mapped ${apName} → ${zone}`, "success");

      apMapForm.reset();

      await refreshAPMapList();
...
// 4. REFRESH AP MAP LIST
async function refreshAPMapList() {
  const apMapList = document.querySelector("#ap-map-list");
  if (!apMapList) return;

  try {
    const apMappings = await staffSyncDb.getAPZoneMappings();

    const apHTML = apMappings
      .map(ap => `
        <div class="ap-item">
          <strong>${ap.ap_name}</strong>
          <br/>
          <strong>Zone:</strong> ${ap.zone}
          <br/>
          <strong>Floor:</strong> ${ap.floor}
          <button 
            class="btn-small" 
            onclick="deleteAPMapping('${ap.id}')"
    ...
    if (!apHTML) {
      apMapList.innerHTML = `
        <p style="color: #666; font-style: italic;">
          No AP mappings yet. Use the form above to map your hotel access points.
        </p>
      `;
      return;
    }

    apMapList.innerHTML = apHTML;

  } catch (error) {
    apMapList.innerHTML = `<p style="color: red;">Error loading APs: ${error.message}</p>`;
  }
}

async function deleteAPMapping(apId) {
  if (!confirm("Delete this AP mapping?")) return;

  try {
    await staffSyncDb.deleteAPZoneMapping(apId);
    showToast("✅ AP mapping deleted", "success");
    await refreshAPMapList();
  } catch (error) {
    showToast(`Error: ${error.message}`, "error");
  }
...
// 5. MANUAL ENGENIUS SYNC
if (engeniusSync) {
  engeniusSync.addEventListener("click", async () => {
    engeniusSync.disabled = true;
    const status = document.querySelector("#engenius-sync-status");

    try {
      status.textContent = "🔄 Syncing...";

      const response = await fetch("/.netlify/functions/engenius-sync", {
        method: "POST",
      ...
      if (response.ok && result.success) {
        ...
        status.textContent = `✅ Last sync: ${timestamp} (${result.trackedStaff} staff)`;

        showToast(
          `✅ Synced ${result.trackedStaff} staff locations`,
          "success"
        );

        await refreshStaffLocations();
      } else {
...
// 6. DISPLAY STAFF LOCATIONS
async function refreshStaffLocations() {
  const locationList = document.querySelector("#location-list");
  if (!locationList) return;

  try {
    const locations = await staffSyncDb.getStaffLocations();

    if (!locations || locations.length === 0) {
      locationList.innerHTML = `
        <p style="color: #666; font-style: italic;">
          No location data yet. 
          <br/>Ensure staff phones are connected to hotel WiFi and have registered their MAC addresses.
          <br/>Then click "Manual Sync Now" or wait for automatic sync (every 5 minutes).
        </p>
      `;
      return;
    }
    ...
    const locationHTML = locations
      .map(loc => {
        const lastSeenTime = new Date(loc.last_seen).toLocaleTimeString();
        const lastSeenDate = new Date(loc.last_seen).toLocaleDateString();

        return `
          <div class="location-item">
            <strong>${loc.staff_profiles?.full_name || "Unknown"}</strong>
            <small>${loc.staff_profiles?.employee_code || "N/A"}</small>
            <br/>
            <strong>📍 Location:</strong> ${loc.zone || "Unknown"}
            <br/>
            <strong>🏢 Floor:</strong> ${loc.floor || "Unknown"}
            <br/>
            <strong>📡 AP:</strong> <code>${loc.ap_name || "Unknown"}</code>
            <br/>
            <strong>⏱️ Last Seen:</strong> ${lastSeenTime} (${lastSeenDate})
          </div>
        `;
      })
      .join("");
    ...
    locationList.innerHTML = `<p style="color: red;">Error loading locations: ${error.message}</p>`;
  }
}
...
setInterval(refreshStaffLocations, 30000);

// ============================================================================
// END ENGENIUS LOCATION TRACKING CODE
// ============================================================================
